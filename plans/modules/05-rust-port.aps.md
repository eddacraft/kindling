...
- **Status:** Draft. PREREQUISITE IN PROGRESS (`feat/memory-commands-cutover`): the plugin `/memory` slash commands are being cut from the bundled TS engine to the Rust `kindling` binary (un-blocks deprecating `-core`/`-store-sqlite`/`-provider-local`, which were still consumed only by these commands). This also adds a Rust `forget` (redact) endpoint + CLI verb the commands need. The `console.warn`/`npm deprecate`/source-removal steps remain (credential-gated / 1.0.0-gated).
